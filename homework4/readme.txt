 i. What was your strategy to defeat the target bot(s)? 
    Our strategy involved abusing the network protocol between the client and server, beginning with the assumption that, in a client server architecture, whatever nuanced vulnerabilities existed at the application level would be made obvious at the network level.  That assumption was correct.
 ...
 ii. How did you get to your solution? What triggers did you use?
    As our approach was to implement a secondary mock client, we did not rely on triggers.  Our solution was discovered through traffic analysis in wireshark, where we immediately noticed that every command the client sends accepts a name as the first param - a strong indication that server authentication "state" was open to abuse.  For example, the server will accept commands for any currently authenticated client regardless of origin.
