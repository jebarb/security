...
class CounterstrikeWrapper():
    ...
    def __init__(self, server="localhost", port=8181, against_bots=False,
                 launch=False, local=False, dm=False, phase=1, bot=2):
...
def move_enemy(client, name, enemy_name):
    if client.against_bots:
        return
    pos = client.determine_location(name)
    print(pos, file=sys.stderr, flush=True)
    print(client.enemy_loc, file=sys.stderr, flush=True)
    ...
    while not client.check_winner(name):
        time.sleep(.2)
        # client.move(enemy_name, 3, 3) # This is breaking future queries for some reason...
        # move_enemy_to_mines(client, name, enemy_name)
        time.sleep(.2)
...
def print_map(client):
    for i in client.wall_loc:
        for j in i:
            print(int.from_bytes(j, byteorder='big'), end='', file=sys.stderr, flush=True)
...
def mine_fill(client, name):
    for i in range(1, max(client.MAP_WIDTH+1, client.MAP_HEIGHT+1)):
        for j in range(1, i+1):
            if i < client.MAP_WIDTH+1 and j < client.MAP_HEIGHT+1:
                if client.wall_loc is not None:
                    if int.from_bytes(client.wall_loc[client.MAP_HEIGHT-j][client.MAP_WIDTH-i],
                                      byteorder='big') == 0x63 and not client.against_bots:
                        client.lay_mine(name, client.MAP_WIDTH-i, client.MAP_HEIGHT-j, 6)
                elif client.map is None or\
                        int.from_bytes(client.map[client.MAP_HEIGHT-j][client.MAP_WIDTH-i],
                                       byteorder='big') not in [0x14, 0x15]:
                    client.lay_mine(name, client.MAP_WIDTH-i, client.MAP_HEIGHT-j, 1)
            if j < client.MAP_WIDTH+1 and i < client.MAP_HEIGHT+1:
                if client.wall_loc is not None:
                    if int.from_bytes(client.wall_loc[client.MAP_HEIGHT-i][client.MAP_WIDTH-j],
                                      byteorder='big') == 0x63 and not client.against_bots:
                        client.lay_mine(name, client.MAP_WIDTH-j, client.MAP_HEIGHT-i, 6)
                elif client.map is None or\
                        int.from_bytes(client.map[client.MAP_HEIGHT-i][client.MAP_WIDTH-j],
                                       byteorder='big') not in [0x14, 0x15]:
                    client.lay_mine(name, client.MAP_WIDTH-j, client.MAP_HEIGHT-i, 1)
    if client.wall_loc is None:
        time.sleep(0.2)
        client.wall_loc = client.update(name)


def mine_enemy(client, name, update=True):
    if update:
        time.sleep(0.1)
        client.get_ppl(name)
    ...
    print('enemy token: ' + str(client.enemy_map_id), file=sys.stderr)
    for i in range(-3, 4):
        for j in range(-3, 4):
            if -1 < (client.enemy_loc[0] + j) < (client.MAP_WIDTH - 1) and\
                    -1 < (client.enemy_loc[1] + i) < (client.MAP_HEIGHT - 1):
                client.lay_mine(name, client.enemy_loc[0] + j, client.enemy_loc[1] + i, 1)


def perimeter_walk(client, name):
    # add client position check after mining enemy
    while not client.check_winner(name):
        # bottom right
        move(client, name, client.MAP_WIDTH - 1, client.MAP_HEIGHT - 1)
        mine_fill(client, name)
        mine_enemy(client, name, False)
        # bottom left:
...
if __name__ == "__main__":
    try:
        ...
        # client = CounterstrikeWrapper(server='colosseum.cs.unc.edu', against_bots=True, launch=True)
        client = CounterstrikeWrapper(server='localhost', against_bots=False,
